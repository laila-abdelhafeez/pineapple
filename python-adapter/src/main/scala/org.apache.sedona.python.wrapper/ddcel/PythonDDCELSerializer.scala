...
private[python] class PythonDDCELSerializer extends Serializable {
  ...
  def serialize(entry: DDCELEntry): Array[Byte] = {
    DDCELSerializer(entry).serialize
  }

//  def deserialize(values: Array[Byte]): DDCELEntry
